...
func (s *ServerTCP) Accept(handler func(net.Conn)) error {
	if s.listener == nil {
	...
	for {
		...
		if s.sem != nil {
			...
			default:
				conn.Close()
				s.onError(fmt.Errorf("tcp max conns reached: %d", s.MaxConns))
				continue
			}
		}
		...
		go func() {
			...
			defer func() {
				if r := recover(); r != nil {
					s.onError(fmt.Errorf("tcp handler panic: %v", r))
				}
			}()
			handler(conn)
		}()
...
// onError 通知错误（安全调用）
func (s *ServerTCP) onError(err error) {
	if s.OnError != nil {
		s.OnError(err)
	}
}
