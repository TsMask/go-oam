...
const (
	Service_Stream_FullMethodName = "/grpc.Service/Stream"
)
...
func (c *serviceClient) Stream(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[Message, Message], error) {
...
type Service_StreamClient = grpc.BidiStreamingClient[Message, Message]
...
func (UnimplementedServiceServer) Stream(grpc.BidiStreamingServer[Message, Message]) error {
...
func _Service_Stream_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(ServiceServer).Stream(&grpc.GenericServerStream[Message, Message]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Service_StreamServer = grpc.BidiStreamingServer[Message, Message]
