...
// SNMP 接收终端交互业务处理
func (s SNMP) Session(client *wsModel.WSClient, reqMsg wsModel.WSRequest) {
	// 必传requestId确认消息
	if reqMsg.RequestID == "" {
		msg := "message requestId is required"
		logger.Infof("ws SNMP UID %s err: %s", client.BindUid, msg)
	...
	if err != nil {
		logger.Warnf("ws SNMP UID %s err: %s", client.BindUid, err.Error())
