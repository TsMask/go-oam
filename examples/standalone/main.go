...
func main() {
	...
	go func() {
		for {
			...
			alarm := oam.Alarm{
				...
				AlarmCode:         100,                    // 告警状态码
				AlarmType:         oam.AlarmSeverityEvent, // 告警类型 CommunicationAlarm,EquipmentAlarm,ProcessingFailure,EnvironmentalAlarm,QualityOfServiceAlarm
				AlarmTitle:        "Alarm Test",           // 告警标题
				PerceivedSeverity: oam.AlarmSeverityEvent, // 告警级别 Critical,Major,Minor,Warning,Event
				AlarmStatus:       oam.AlarmStatusActive,  // 告警状态 Clear,Active
			...
			if errs != nil {
				...
				fmt.Println("==> Send ok AlarmSeq:", alarm.AlarmSeq)
			}
			...
			uenb := oam.UENB{
				...
				Result: oam.UENBResultAuthSuccess, // 结果值
				Type:   oam.UENBTypeAuth,          // 终端接入基站类型
			}
	...
	o.RouteAdd(func(r gin.IRouter) {
		// 网管接收端收告警
		oam.AlarmReceiveRoute(r, func(alarm oam.Alarm) {
			fmt.Println("<== Receive Alarm", alarm)
		})
		// 网管接收端收终端接入基站
		oam.UENBReceiveRoute(r, func(uenb oam.UENB) {
			fmt.Println("<== Receive UENB", uenb)
		})
		// 网管接收端收话单
		oam.CDRReceiveRoute(r, func(cdr oam.CDR) {
			fmt.Println("<== Receive CDR", cdr)
		})
		// 指标发送测试
		oam.KPITimerStart("http", "192.168.5.58:29565", "neUid", 10*time.Second)
		// 网管接收端收KPI
		oam.KPIReceiveRoute(r, func(kpi oam.KPI) {
